...
public interface NoticeRepository extends JpaRepository<Notice, String>{
	...
	@Query(value = """
		    ...
		    WHERE 
		    	(:#{#filterRequest.noticeCd} IS NULL 
		            OR n.notice_cd LIKE %:#{#filterRequest.noticeCd}%)
		        AND(:#{#filterRequest.employeeDp} IS NULL 
		            OR d.common_nm LIKE %:#{#filterRequest.employeeDp}%) 
		        AND (:#{#filterRequest.employeeNm} IS NULL 
		            OR e.employee_nm LIKE %:#{#filterRequest.employeeNm}%) 
		        AND (:#{#filterRequest.noticeTt} IS NULL 
		            OR n.notice_tt LIKE %:#{#filterRequest.noticeTt}%) 
		        AND (
		            :#{#filterRequest.startDate} IS NULL 
		            OR :#{#filterRequest.endDate} IS NULL 
